...
const VARIANT =
    (__ENV.VARIANT || 'control').toLowerCase();

if (!Number.isInteger(VUS) || VUS < 1 || VUS > 499) {
    throw new Error('VUS debe estar entre 1 y 499.');
}

if (VARIANT !== 'control' && VARIANT !== 'paged') {
    throw new Error(
        'VARIANT debe ser control o paged.'
    );
}
...
const ENDPOINT =
    VARIANT === 'control'
        ? '/api/activities'
        : '/api/activities/paged?page=0&size=100';
...
const RESULT_JSON =
    __ENV.RESULT_JSON ||
    `experimentos/semana8-paginacion-activities/resultados/${VARIANT}.json`;
...
export default function (data) {
    ...
    if (__VU === 1 && __ITER === 0) {
        console.log(
            `PAGINATION_${VARIANT.toUpperCase()}_START ` +
            `epochMs=${Date.now()}`
        );
    }
    ...
    activityGetBlocked.add(
        response.timings.blocked
    );

    activityGetConnecting.add(
        response.timings.connecting
    );

    activityGetSending.add(
        response.timings.sending
    );

    activityGetWaiting.add(
        response.timings.waiting
    );

    activityGetReceiving.add(
    ...
    check(response, {
        ...
        'GET activities devuelve conteo esperado':
            () => countOk,
    });
}
...
export function teardown() {
    console.log(
        `PAGINATION_${VARIANT.toUpperCase()}_END ` +
        `epochMs=${Date.now()}`
    );
}
